...
class AXI4StreamAsyncQueueWithControlIO(ctrlBits: Int) extends Bundle {
    ...
    override def cloneType: this.type = AXI4StreamAsyncQueueWithControlIO(ctrlBits).asInstanceOf[this.type]
}
object AXI4StreamAsyncQueueWithControlIO{
  def apply(ctrlBits: Int): AXI4StreamAsyncQueueWithControlIO = new AXI4StreamAsyncQueueWithControlIO(ctrlBits)
}

class AXI4StreamAsyncQueueWithControlBlock(params: AXI4StreamAsyncQueueWithControlParams)(implicit p: Parameters) extends AsyncQueueWithControlBlock[AXI4MasterPortParameters, AXI4SlavePortParameters, AXI4EdgeParameters, AXI4EdgeParameters, AXI4Bundle](params) with AXI4DspBlock {
  override val mem = None
}

abstract class AsyncQueueWithControlBlock[D, U, E, O, B <: Data] (params: AXI4StreamAsyncQueueWithControlParams) extends LazyModule()(Parameters.empty) with DspBlock[D, U, E, O, B] {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    // IOs
    val io = IO(new AXI4StreamAsyncQueueWithControlIO(params.ctrlBits))

    // AsyncQueue
    val asyncQueue = Module(new AsyncQueueModule(chiselTypeOf(Cat(io.in_ctrl, in.bits.data)), depth = params.depth, sync = params.sync, safe = params.safe))
...
object AXI4StreamAsyncQueueWithControlApp extends App
{
  ...
  val lazyDUT = LazyModule(new AXI4StreamAsyncQueueWithControlBlock(params) with AXI4StreamAsyncQueueWithControlStandalone)
