...
import dspblocks._ // included because of SimpleDspQueue
...
class SimpleDspQueue(val params: SimpleDspQueueCustomParams, beatBytes: Int) extends LazyModule()(Parameters.empty) {
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    val queuedStream = Module(new QueueWithSyncReadMem(chiselTypeOf(streamIn.bits), entries = depth, useSyncReadMem = params.useSyncReadMem, useBlockRam = params.useBlockRam, flow = false, pipe = true))
...
{
  val params: SimpleDspQueueCustomParams = SimpleDspQueueCustomParams(queueDepth = 16534) //SimpleDspQueueCustomParams(queueDepth = 131074, progFull = true)
